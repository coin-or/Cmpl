	...
	def load(self, name=None):
		...
		try:
			...
			if self.__serverMode == STANDALONE_SERVER:
				if self.__cmplUrl:
					self.__cmplServer = xmlrpclib.ServerProxy(self.__cmplUrl)
			...
			if self.__serverMode == CMPL_GRID:
				...
				if self.__cmplGridSchedulerUrl:
					self.__cmplGridScheduler=xmlrpclib.ServerProxy(self.__cmplGridSchedulerUrl)
			...
			#raise CmplException("Internal error, cannot read dump: "+str(sys.exc_info()[1]))
                        raise CmplException("No CmplServer object exists. Please send your problem to a CmplServer before knocking, retrieving or canceling it.")
