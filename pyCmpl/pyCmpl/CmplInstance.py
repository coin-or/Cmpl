...
class CmplInstance(object):
	...
	def cmplInstanceStr(self, cmplFileName, optList, dataString, jobId):
		...
		self.__cmplDataList.update({ os.path.basename(os.path.splitext(self.__cmplFile)[0])+".cdat" :   self.__dataString  })
		...
		try:
				...
				if line.startswith('%data'):
					...
					if tmpName != "":
						if not os.path.isfile(tmpName):
							raise CmplException("cmplDataFile " + tmpName + " does not exist."  )
						...
						tmpName1 = os.path.basename(tmpName)		
						if os.path.basename(tmpName1) in self.__cmplDataList:
							tmpName1 = str(random.randint(100000, 999999)) + tmpName1	
						
						self.__cmplDataList.update({ tmpName1 :  tmpString  })
						
						lines[lineNr] = line.replace(tmpName, tmpName1) + "\n"
			...
			raise CmplException("IO error for cmpl file " + self.__cmplDataFile + ": "+e)
