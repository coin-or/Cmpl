...
%token OBJ_TO TRANSPOSE ERRMSG USRMSG
/*%token VEKT_BEG*/
...
%type <exp> set_expr const_set_expr tupel_expr
...
const_def:	ul_sym_vekt const_type_def
				 ...
				 vekt_def ';'					{ /* Unterscheidung ob linke Seite normales Symbol oder Tupel */
				 								  if ($1.dim_t_rank1 == 0)
												  {
													  ...
													  if ($1.dim_out >= 0 && $5.dim_out >= 0 && $1.dim_out != $5.dim_out)
													  ...
													  symbol_comp_str *symp = $1.symp();
													  if (!(symp->isvalid()))
														PARSE_CONTEXT.init_symbol(&$1, PTR_TP_CONST, $1.tp, false);
													  else
														PARSE_CONTEXT.test_tpk_symbol(symp, $1.tp, true);

													  if ($1.sub_sym_ind < 0)
													  {
														COMP_SUB_PAR_CNT(DEF_CONST, SYM_MAIN, $2, 1);
														COMP_INT(symp->symno * MAX_ANZ_IND_1 + $1.dim_in);
													  }
													  else
													  {
														COMP_SUB_PAR_CNT(DEF_CONST, SYM_SUB, $2, 1);
												  ...
												  else
												  {
													  ...
													  symbol_comp_str **tupel_syms = (symbol_comp_str **) ($1.symtab);
		...
		|	REF ul_sym_vekt const_type_def
				...
				vekt_def ';'					{ /* Ausdruck auf der rechten Seite muss lvalue sein (und damit auch Symbol enthalten) */
												  ...
												  if (!(symp2->isvalid()))
												    PARSE_CONTEXT.init_symbol(&$2, PTR_TP_CONST, $6.tp, false);
												  else
												  	PARSE_CONTEXT.test_tpk_symbol(symp2, $6.tp, true);

												  if ($2.sub_sym_ind < 0)
												  {
				 								  	COMP_SUB_PAR_CNT(DEF_CONST, SYM_MAIN_REF, $3, 1);
				 								  	COMP_INT(symp2->symno * MAX_ANZ_IND_1 + $2.dim_in);
												  }
												  else
												  {
				 								  	COMP_SUB_PAR_CNT(DEF_CONST, SYM_SUB_REF, $3, 1);
...
d_var_vekt:	d_undef_sym_vekt					{ COMP_PAR(LIT_VAL_SHORT, $1.dim_in);
												  COMP_SUB(SET_INDEXES, IND_GIVEN);
												  PARSE_CONTEXT.init_symbol(&$1, PTR_TP_VAR, TPK_VAR, false);
												  COMP(DEF_VAR, 1);
...
//		|	'(' const_expr ')'					{ $$ = $2; $$.symtab = NULL; $$.lval = false; }
		...
		|	DEFS_TMP							{ COMP(FETCH_TMP, 1);
												  ...
												  $$.set(TPK_VALUE, 0); }	//TODO: Symbol, lvalue, 0 indizes (alles vorläufig)
...
function_expr:	FCT_MAX start_list				{ $2 = (int) token::FCT_MAX; }
		...
		|	DIM '(' bel_sym_vekt ')'			{ /*COMP_PAR(LIT_VAL_SHORT, $3.dim_in);
												  ...
												  COMP_INT($3.symp()->symno * MAX_ANZ_IND_1 + $3.dim_in);
												  $$.set(TP_INT, 0); }
...
subsym_expr: subsym_pre SUBSYM_OP subsym_sub d_vekt
												{ $$ = PARSE_CONTEXT.parse_type_dev_subsym_op(&$1, $3);
												  ...
												  {
												  	$$.set_dim($4.dim_in, $4.dim_out, $4.dim_fr_end);
												  	PARSE_CONTEXT.init_symbol(&$$, PTR_TP_CONST, TPK_VALUE, false);
												  } }
		;
...
const_elem:	all_expr
		...
		|	subsym_pre SUBSYM_OP
				subsym_sub d_vekt				{ EXPSTR exp = PARSE_CONTEXT.parse_type_dev_subsym_op(&$1, $3);
												  if ($4.dim_in > 0)
												  {
												  	if (exp.dim_out >= 0 && exp.dim_out != $4.dim_in)
														PARSE_CONTEXT.output_error_parse(ERROR_TYPE_COMP, ERROR_LVL_NORMAL, "array dimensions don't match");/*TODO*/
...
val_list_elem:										{ COMP_PAR(LIT_SET, TP_SET_ALL);
		...
		| const_set_expr							{ // Undefinierte Symbole hier nicht erlaubt
													  if ($1.dim_t_undef > 0)
													  	PARSE_CONTEXT.output_error_parse(ERROR_TYPE_COMP, ERROR_LVL_NORMAL, "undefined symbols in list");/*TODO*/
													  $$ = $1;
													  $$.set_dim(1, ($1.dim_out==0 && (TYPECLASS($1.tp)==TPK_SET || TP_IS_TUPELSET($1.tp)) ? 1 : 0), 0, $1.dim_out,
...
line_name_sym: SYMBOL_LINENAME						{ $$ = $1;
													  $$.tp = TPK_VALUE;
													  if (!($1.symp()->isvalid()))
														PARSE_CONTEXT.init_symbol(&$1, PTR_TP_LINENAME, TPK_VALUE, false); }
		| SYMBOL_UNDEF								{ $$ = $1;
													  $$.tp = TPK_VALUE;
													  PARSE_CONTEXT.init_symbol(&$1, PTR_TP_LINENAME, TPK_VALUE, false); }
		;
...
loop_for: loop_for_var loop_for_assign
			loop_for_set						{ /* Unterscheidung ob Iteration über Set oder lokale Definition */
												  ...
												  unsigned short sub = ($2 == 2 || TYPECLASS($3.tp) == TPK_SET ? START_LOOP : START_ASSIGN);
												  /* Art und Name der Laufkonstante eintragen */
												  $1.tp = (sub == START_LOOP ? TPK_VALUE : TYPECLASS($3.tp));
												  PARSE_CONTEXT.set_loop_sname(sub, &$1);
												  /* Wenn $2 == 0 (alte Zuweisung): Wenn $3 != TPK_SET (einfacher Wert), dann Fehler; sonst Warnung */
												  if ($2 == 0) PARSE_CONTEXT.out_warn_old_assign((TYPECLASS($3.tp) != TPK_SET));
												}
												//TODO: Tokenwert $1 auf expr; an set_loop_sname den Symbolnamen übergeben
		;
...
loop_for_var: SYMBOL_UNDEF						{ $$ = $1; /*strcpy((char *) (GLV->c_getmem(sizeof(char) * strlen(GLV->last_str))), GLV->last_str);*/ }
												//TODO: Tokenwert sollte expr sein: als temporärer Parameter definieren/testen; $$ = $1;
												//TODO: fuer lokale Definition sollten auch Indizes erlaubt sein
												//TODO: last_str entfällt?
		;
