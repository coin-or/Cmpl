...
namespace cmpl
{
	...
	void Compiler::init_symbol(EXPSTR *exp, enum ptrtp symtp, enum etp tpk, bool local)
	{
	...
	void Compiler::insert_subsymbol(int tp_sub_tab, const char *sname, enum etp tpk, int dim)
	{
		...
		if (symno < 0)
		{
			...
			init_symbol(&exp, PTR_TP_CONST, tpk, false);
		}
	}
	...
	loop_ctrl_str *Compiler::set_loop_comp(unsigned short sub, unsigned short mod)
	{
		...
		set_loop_sname(sub, (EXPSTR *)NULL);
	...
	void Compiler::set_loop_sname(unsigned short sub, EXPSTR *exp)
	{
		// Falls Expression, dann muss Symbol darin enthalten und undefiniert sein (durch die Grammatik gesichert)
		symbol_comp_str *symp = (exp ? exp->symp() : (symbol_comp_str *)NULL);
		...
		loopp->s_name = sname;
		loopp->sym_ind = (symp ? exp->sym_ind : -1);

		// Symbol fertig initialisieren
		if (symp)
		{
			symp->symno = glvar->tmp_level - 1;
			init_symbol(exp, PTR_TP_CONST, exp->tp, true);
		}
	...
	void Compiler::unset_loop_comp()
	{
		...
		while (1)
		{
			PROTO_OUT_2("\nunset_loop_comp: %d", (glvar->tmp_level - 1));
			...
			if (symp)
			{
				PROTO_OUT_3("\n    symbol #%d (%s) goes out of scope", symp->symno, symp->s_name);
				symp->valid = false;
			}
	...
	//TODO: nicht als "symbol_comp_str *", sondern als Indizes in Symboltabelle
	symbol_comp_str ** Compiler::tupel_assign_symbols(int *cnt)
	{
		...
		val_str *p = glvar->prog + (--(glvar->comp_ptr));
		...
		// Im Zwischencode steht als letztes CONSTRUCT_TUPEL; davor literaler Listenzähler,
		// 	davor FETCH_CONST/FETCH_TMP für die Elementsymbole im Tupel
	...
	/* generates the command for a tupel assign to the intermediary code */
	//TODO: nicht als "symbol_comp_str *", sondern als Indizes in Symboltabelle
	t_addr Compiler::comp_tupel_assign(symbol_comp_str **syms, int cnt, bool local)
	{
		// Kommando für Tupel-Assign compilieren
		// (holt dann in der Ausführung Tupel vom Stack, und weist dessen Bestandteile den Symbolen hier zu)
		// Für bereits definierte Symbole Typklasse prüfen (muss TPK_VALUE sein), für undefinierte eintragen
		// undefinierte Symbole als globale/lokale Symbole definieren

		// Zurück Position im Zwischencode nach compilierten Kommando
	}
