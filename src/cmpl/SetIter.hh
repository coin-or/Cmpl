...
namespace cmpl
{
	...
	public:
		/**
		 * counts the elements in the set or tupelset.
		 * @param setp		set
		 * @return 			number of elements
		 */
		inline long set_len(val_str *setp)		{ return set_len(setp, false); }

		/**
		 * counts the elements in the set or tupelset.
		 * @param setp		set
		 * @param tps_enum	if <code>setp</code> is a tupelset it contains a <code>TupelsetEnum</code> instead of <code>Tupelset</code>
		 * @return 			number of elements
		 */
		long set_len(val_str *setp, bool tps_enum);

		/**
		 * counts the elements in the set of a specific value type (not for tupelsets).
		 * @param setp		set
		...
		/**
		 * counts the elements in the intersection of two sets (not for tupelsets).
		...
		/**
		 * tests if a specific value is an element of the set or tupelset.
		 * @param setp		set
		...
		/**
		 * construct a set from an enumeration of elements.
		 * @param glvar     CmplCore object of CMPL
		 * @param arr		array with elements
		 * @param len		length of <code>arr</code>
		 * @param glmem     global allocation of the memory
		 * @param set		return of the constructed set
		 */
		static void constructSetEnum(CmplCore *glvar, val_str *arr, int len, bool glmem, val_str *set);

		/**
		 * writes a textual form of the set in a string.
		 * @param glvar     CmplCore object of CMPL
		 ...
		 */
		static char *asString(CmplCore *glvar, char *dest, val_str *set, int maxlen);
	};
	...
	public:
		 ...
		 * constructor
		 */
		inline SetIterator(): SetUtil() { tps_enum = false; tps_iter = NULL; }

		/**
		 * constructor
		 * @param m		CmplCore object of CMPL
		 */
		inline SetIterator(CmplCore *m): SetUtil(m) { tps_enum = false; tps_iter = NULL; }
		 ...
		 */
		inline SetIterator(CmplCore *m, val_str *setp): SetUtil(m) { set = *setp; tps_enum = false; tps_iter = NULL; }

		/**
		 * constructor
		 * @param m		CmplCore object of CMPL
		 * @param setp	set to iterate
		 * @param tpe	<code>setp</code> contains a <code>TupelsetEnum</code>
		 */
		inline SetIterator(CmplCore *m, val_str *setp, bool tpe): SetUtil(m) { set = *setp; tps_enum = tpe; tps_iter = NULL; }


	/****** variables for iteration ****/
	public:
		val_str set;							///< set to iterate (must be of one type TP_SET_* or TPK_TUPELSET_N*)
	...
	private:
		...
		bool tps_enum;							///< only used in elements of <code>tps_iter</code>: <code>set</code> contains a <code>TupelsetEnum</code>
		TupelsetIteratorBase *tps_iter;			///< iterator object only used for iterating a <code>Tupelset</code> or <code>TupelsetEnum</code>
					// TODO: Tupelset: array of iterators for the <code>ind()</code> elements of a tupelset, only used while iterating a tupelset
					// TODO: TupelsetEnum: darin gegebenenfalls: Iterationskontext für enthaltenes Tupelset
			//TODO: Freigabe Iterationskontext, wenn Iteration nicht bis zum Ende läuft, sondern abgebrochen wird
			//TODO: Zusaetzliche Funktion release() für Abschluss Iteration, darin Freigabe Iterationskontext; Aufruf release()
	...
	public:
		...
		/**
		 * returns pointer to the current value of the iteration.
		 * @return			current value
		 */
		inline val_str *curVal()	{ return &cur_val; }

		/**
		 * returns wether the iteration has reached the end of the set.
		 * @return			true if iteration is ended
		 */
		inline bool isEnd()			{ return set_end; }
