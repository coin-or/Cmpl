...
namespace cmpl
{
	...
	bool SetIterator::iter_set_start_intern(val_str *setp, bool dirfwd)
	{
		...
		else	/* Hier muss SET_USE_ARRAY gelten */
		{
			...
			{
				array_ind_dir = (array->a_dir.cnt > 0 ? array->a_dir.len-1 : -1);
				array_ind_num = (array->a_num.cnt > 0 ? array->a_num.len-1 : -1);
				array_ind_str = (array->a_str.cnt > 0 ? array->a_str.len-1 : -1);
			}
