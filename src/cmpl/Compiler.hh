...
namespace cmpl
{
	...
	public:
		 ...
		 */
		int search_symbol(char *str, int *ip);
		 ...
		 */
		int insert_symbol(char *name, enum etp tpclass);

		/**
		 * sets or tests the type class for a global symbol
		 * @param symnr		number of the global symbol; if it already has a type class it must be the same, if it has not the type class is set
		 * @param tpclass	type class
		 */
		void set_test_tpcl_symbol(int symnr, enum etp tpclass);
		 ...
		 */
		void init_loop_comp(unsigned short nr, bool base, enum looptp loop_tp, char *lname);
		 ...
		 * @param sname		reference name of the loop / NULL: no reference name
		 * @param inc		change to next loop context
		 * @return			pointer to the loop context
		 */
		loop_ctrl_str *set_loop_comp(unsigned short sub, unsigned short mod, char *sname, bool inc);
		 ...
		 * @param sname		reference name of the loop / NULL: no reference name
		 */
		void set_loop_sname(unsigned short sub, char *sname);
		 ...
		 */
		void linename_repl(char *n_dest, char *n_src);
		 ...
		 * @return				appropriate token
		 */
		CMPL_Parser::token_type test_symbol(CMPL_Parser::semantic_type *yylvalp);
		 ...
		 * @return			type of result expression
		 */
		enum etp parse_type_dev_num_op(int sub, enum etp t1, enum etp t2);
		 ...
		 * @return			type of result expression
		 */
		enum etp parse_type_dev_cmp_op(int sub, enum etp t1, enum etp t2);
		 ...
		 */
		enum etp parse_type_dev_lit_set(enum etp settp, enum etp t1, enum etp t2, enum etp t3);
