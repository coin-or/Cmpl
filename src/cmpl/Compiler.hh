...
namespace cmpl
{
	...
	public:
		 ...
		 */
		void init_symbol(EXPSTR *exp, enum ptrtp symtp, enum etp tpk, bool local);
		 ...
		 */
		void set_loop_sname(unsigned short sub, EXPSTR *exp);
		 ...
		 * @return			array of the symbols in the tupel, the array length is cnt
		 */
		symbol_comp_str ** tupel_assign_symbols(int *cnt);

		/**
		 * generates the command for a tupel assign to the intermediary code.
		 * @param syms		array of the symbol pointers of the left hand side of the assign
		 * @param cnt		count of symbol pointers in syms
		 * @param local		define undefined symbols as local
		 * @return			current address in the intermediary code
		 */
		t_addr comp_tupel_assign(symbol_comp_str **syms, int cnt, bool local);
