...
namespace cmpl
{
	...
	{
		forall_var(&cmpl::IntFuncLineBase::fdef_linebeg);
	...
	int IntFuncLineBase::fline_end()
	{
		...
		dp = (dest_str *) glvar->c_getmem(sizeof(dest_str) + glvar->gl_mline_anz*sizeof(l_elem_str));
		if(!glvar->dest_grd) {
		...
		for(dp1=glvar->dest_grd; dp1!=dp; dp1 = dp1->d_nextline) {
			if(strcmp(dp1->d_name, glvar->name_s_line) == 0) {
				...
				snprintf(glvar->errbuf, ERRBUFLEN, ERROR_LINE_NAME_DUP_REPL,
													dp1->d_name, glvar->name_s_line);
		...
		dp->d_anz_val = glvar->gl_mline_anz;
		...
		if(glvar->aufruf_p->is_no_reduct || ftest_dest()) {
	...
	int IntFuncLineBase::ftest_dest()
	{
		...
		/* Test auf Zeilen mit aequivalenten Koeffizienten */
		{
			...
			if(dp != glvar->dest_akt) {	/* proportionale Zeile gefunden */
				PROTO_OUT_4("\n   proportional constraints: %s # %s (multiplied by :%lf)",
	...
	int IntFuncLineBase::make_lrange(dest_str *lp, val_str uv, val_str ov, bool nu, bool no)
	{
		...
		PROTO_OUT_4("\n   constraint %s : range %lf - %lf", lp->d_name, ub, ob);
		...
		if(IS_EQ(rg, 0.)) {
			...
			if(lp->d_is_range) {
				lp->d_is_range = false;
#ifndef RANGE
				lp->d_range_vp->v_is_koeff = false;
				lp->d_anz_val--;
#endif /* RANGE */
			}
			return(0);
		...
		if(lp->d_is_range) {
			...
			if(ip == -1) {
				ip = insert_symbol(str, TPK_VAR);
				ind = ip / MAX_ANZ_IND_1;
			}
			...
			*dp = *lp;
			...
			dp->d_valp[n].le_col = glvar->akt_defnum;
