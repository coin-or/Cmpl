...
namespace cmplUtil {
    ...
    void ArgError::getMessage() {
        ...
        string errM;
        if (_cmplHeader)
            errM="CMPL header error ";
        else
            errM="CMPL argument error ";

        if ( _eArg.length()!=0)
                cerr << errM << "at " << _eArg << ": " << _message << endl<< endl;
            else
                cerr << errM << " : " << _message << endl<< endl;
        if (!_cmplHeader)
            usage(1);
    } // end getMessage
    ...
    void ArgError::usage(int mode) {
        ...
        uOut << "Usage: cmpl [options] [<input file>]" << endl << endl ;
        ...
        uOut << " -cd                 : no warning at multiple parameter definition" << endl;
        uOut << " -ca                 : no warning for deprecated '=' assignment" << endl;
        ...
        uOut << " -ff                 : format option: generate free MPS" << endl;
