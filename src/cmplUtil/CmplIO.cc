...
namespace cmplUtil {
   ...
   CmplIO::CmplIO( cmpl::CmplCore *m , CmplInstance *mi) {
    ...
    int CmplIO::solveModel() {
        ...
        if (CmplTools::lowerCase(args->solverName())=="glpk" || CmplTools::lowerCase(args->solverName())=="gurobi") {
            runGlpkOrGurobi();
        } else {
            runOSSolverservice();
        }
    ...
    /* **************** runGlpk *********************** */
    int CmplIO::runGlpkOrGurobi() throw (CmplException) {
        ...
        if (solverStatus==PROG_DONT_RUN) throw CmplException("can not connect the solver: "+( CmplTools::lowerCase(args->solverName())=="glpk" ? glpkPath.toStdString(): "Gurobi"));
    ...
    int CmplIO::runSolver(int actionMode) {
        ...
        if (CmplTools::lowerCase(args->solverName())=="glpk") {
        ...
        } else  if (CmplTools::lowerCase(args->solverName())=="gurobi") {
