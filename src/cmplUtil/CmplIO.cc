...
namespace cmplUtil {
   ...
   CmplIO::CmplIO( cmpl::CmplCore *m , CmplInstance *mi) {
    ...
    }//end writeCmplOutput obsolet


    /* *********** readCmplOutput *********** */
    string CmplIO::readCmplOutput() {
        return cmplOutput;
    }//end readCmplOutput obsolet
    ...
    int CmplIO::runSolver(int actionMode) {
        ...
        if (args->solverName()=="glpk") {
            ...
            QString tmpcFProblem=cFProblem;
#ifdef WIN
            progString +=  "\""+cmplIOPath+"\"gurobiPython.bat "+tmpWorkingDir.replace(" ","%")+"  "+ tmpcFProblem.replace(" ","%")+".mps " + tmpcFProblem.replace(" ","%") + ".gsol " +  QString::fromStdString(args->nonCoinSolverOpts());
#else
            progString +=  "\""+cmplIOPath+"\"gurobiPython "+cmplIOPath+" "+tmpWorkingDir.replace(" ","%")+"  "+ tmpcFProblem.replace(" ","%")+".mps " + tmpcFProblem.replace(" ","%") + ".gsol " +  QString::fromStdString(args->nonCoinSolverOpts());
#endif
    ...
    bool CmplIO::writeConfigFile(int mode) {
        ...
        if (mode==ACTION_SOLVE_CPLEX) {
            cplexConfig= "read " + cFProblem.toStdString() + ".mps\n";
            cplexConfig+=args->nonCoinSolverOpts();
            cplexConfig+= "opt\n";
            cplexConfig+="write  " + cFProblem.toStdString() +".gsol sol \n" ;
            cplexConfig+= " quit \n \n" ;
        }
