        ...
        if (cplFile) {
            while ( getline(cplFile , line ) ) {
                    ...
                    while (iss) {
                        ...
                        if (!subs.empty() && !CmplTools::stringStartsWith(subs,"%opt")) {
                            ...
                            if( !(tmpOsol.solver=="cbc" || tmpOsol.solver=="glpk" || tmpOsol.solver=="gurobi" || tmpOsol.solver=="scip" || tmpOsol.solver=="cplex") )
                                throw ArgError(true, subs, "Only CBC, CPLEX, GLPK, Gurobi or Scip solver parameters permitted in this version." );
                        }
                    }
                ...
                if (CmplTools::stringStartsWith(line,"%display")) {
                    ...
                    if (optList.size()<2) throw ArgError(true, "%display", "Either var or con are defined" );
                    if (optList.size()<3) throw ArgError(true, "%display", "No variable or constraint name defined" );
                    ...
                    } else throw ArgError(true, optList[1].toStdString(), "Wrong display parameter" );
                }
