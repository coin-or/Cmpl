...
namespace cmplUtil {
    ...
    bool cfResult::getResult() {
        ...
        readInstance();
        if (CmplTools::lowerCase(glvar->args->solverName())=="glpk" || CmplTools::lowerCase(glvar->args->solverName())=="gurobi") {
...
#define rOut (outType==2 ? rFile : cout )
    ...
    bool cfResult::readGSolFile() {
        ...
        if (CmplTools::lowerCase(glvar->args->solverName())=="glpk" ) {
            resultStatus.serviceName="GLPK";
            resultStatus.solverName="GLPK";
        } else if( CmplTools::lowerCase(glvar->args->solverName())=="gurobi" ){
        ...
        while (!cFOptFile.atEnd()) {
            ...
            if (lineCounter==2) {
                if (solList[0]=="2" || solList[0]=="5"  )
                    ...
                    resultStatus.generalStatus="unfeasible";
