...
namespace cmpl
{
	...
	// defines the module "inputFunctions" that is implemented in class "InputFunctionsMod".
	// to register this module you have to insert "MODULE_CLASS_REG(1, inputFunctions)" to file "modules.reg"
	MODULE_CLASS( inputFunctions, InputFunctionsMod )
	 ...
	 * run module function
	 */
	void InputFunctionsMod::run()
	{
		PROTO_OUTL("Start run inputFunctions module " << moduleName());

        InputFunctions *ext = new InputFunctions(this, _namespace);
		...
		PROTO_OUTL("End run inputFunctions module " << moduleName());
	}
     ...
     */
    void InputFunctionsMod::regModOptions(vector<CmdLineOptList::RegOption> &modOptReg)
    {
     ...
     */
    bool InputFunctionsMod::parseOption(int ref, int prio, CmdLineOptList::SingleOption *opt)
    {
            ...
            case OPTION_FUNCTION_NAMESPACE:
                ...
                if (!opt->neg()) {
                    if (opt->size() > 0) {
                        ...
                        _namespace.push_back(data()->globStrings()->storeInd("input"));
                    }
                }
                return true;
        }
     ...
     */
    void InputFunctionsMod::usage(ostream& s)
    {
        ExtensionModule::usage(s);

        s << "  -namespace [<namespace>]      define function symbols within this namespace (defaults to 'input') instead without namespace" << endl;
    }
	 ...
	 */
	ExtensionBase *InputFunctionsMod::createFromSerialization(MainData::SerializeInfo& si, const MainData *data, int subCnt, int lc, string &rline)
	{
		InputFunctions *ext = new InputFunctions(this);
