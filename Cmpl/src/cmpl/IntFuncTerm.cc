...
namespace cmpl
{
	...
	void IntFuncTermEnd::exec(val_str *vp)
	{
		...
		if(glvar->term_var_bl.tv_anz_var==0) {
			...
			while(anz>1) {
                if((anz = search_rep_var_prod()) == 1)
                    break;
                if(tv_test_swap_fakt())
                    break;
#ifdef PROTO
	...
	void IntFuncTermEnd::fkum_vkoeff()
	{
		...
		v_koeff = glvar->gl_mline_vp + vp->v_defnum;
		v_is_koeff = glvar->gl_mline_fp + vp->v_defnum;

		if (*v_is_koeff == 2) {
			v2 = (f2 = (v_koeff->t != TP_DBL)) ? ((double)v_koeff->v.i) : v_koeff->v.f;
		}
		else {
			f2 = true;
			v2 = 0;
		...
		if (*v_is_koeff == 0)
			glvar->gl_mline_num[glvar->gl_mline_cnt_var++] = vp->v_defnum;

		if (not0) {
			if (*v_is_koeff != 2)
				glvar->gl_mline_cnt_not0++;
			*v_is_koeff = 2;
		}
		else {
			if (*v_is_koeff == 2)
    ...
    void IntFuncTermEnd::make_test_var(symbol_str *sym_p, int mi, var_str **vpp, int symnum)
	{
		array_str *arr_p;
		union ptr_sym_u up;
		array_ins_info_str ret_ins_info;
		ind_inf_one_str *inf_p;
        bool found, nw = false, chg = false;
        var_str *pdir, *pnum, *pstr;
		int n;
		...
		for (n=0, inf_p=glvar->ind_inf_ptr->i_ind; n<mi; n++, inf_p++)
		{
			...
			else
			{
				...
				{
                    /* Pruefen, ob sich beim Einfuegen Pointer aendern */
                    if (!nw) {
                        pdir = arr_p->a_dir.array.v;
                        pnum = arr_p->a_num.array.v;
                        pstr = arr_p->a_str.array.v;
                    }

					up = insert_ind_elem(arr_p, inf_p->cur_val, inf_p->cur_tp, NULL, &ret_ins_info);

                    chg = (!nw && (pdir != arr_p->a_dir.array.v || pnum != arr_p->a_num.array.v || pstr != arr_p->a_str.array.v));
				}
				*vpp = up.v;
			}
		}
        ...
        if (chg) {
            /* Pointer auf Elemente des Symbols koennten ungueltig geworden sein, deshalb neu bestimmen */
            for (n = 0; n < glvar->term_var_bl.tv_anz_var; n++) {
                set_renew_tv_varp(glvar->term_var_bl.tv_vars + n, symnum, arr_p);
            }

            for (struct var_split_str *p = glvar->var_split_grd; p; p = p->vs_next_vs) {
                set_renew_tv_varp(&(p->vs_ivar), symnum, arr_p);
                set_renew_tv_varp(&(p->vs_bvars), symnum, arr_p);
            }

            for (struct var_prod_str *p = glvar->var_prod_grd; p; p = p->vp_next_vp) {
                set_renew_tv_varp(&(p->vp_fakt1), symnum, arr_p);
                set_renew_tv_varp(&(p->vp_fakt2), symnum, arr_p);
                set_renew_tv_varp(&(p->vp_prod), symnum, arr_p);
            }
        }
	}

    /* Setzt den Pointer in tvp->tv_varp neu, fuer passende Symbole und Indizes */
    void IntFuncTermEnd::set_renew_tv_varp(struct term_var_str::tv_str *tvp, int symnum, array_str *arr_p)
    {
        if (tvp->tv_sym_num == symnum) {
            /* Alle Indizes ausser dem letzten muessen uebereinstimmen */
            int mi = symnum % MAX_ANZ_IND_1;
            int n;
            ind_inf_one_str *inf_p;
            val_str *ind;

            for (n=0, inf_p=glvar->ind_inf_ptr->i_ind, ind=tvp->tv_inds; n<mi-1; n++, inf_p++, ind++) {
                if (inf_p->cur_tp != ind->t || inf_p->cur_val != ind->v.i)
                    return;
            }

            /* Pointer auf Element arr_p neu bestimmen */
            bool found;
            array_ins_info_str ret_ins_info;
            union ptr_sym_u up = search_ind_elem(arr_p, ind->v.i, ind->t, &found, &ret_ins_info);
	...
	int IntFuncTermEnd::make_prod_var(Compiler &cmplcomp, struct term_var_str::tv_str *tvp, struct term_var_str::tv_str *tvf1, struct term_var_str::tv_str *tvf2, int sn)
	{
        ...
        make_test_var(glvar->symbol_akt_p, in, &vp, ip);
		...
		if (glvar->gl_mline_cap < glvar->akt_defnum) {
			...
			memset(glvar->gl_mline_fp + glvar->gl_mline_cap, 0, inc * sizeof(char));
