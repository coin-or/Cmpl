...
namespace cmplUtil {
    ...
    /* **************** gsolFile ************* */
    string CmplArgs::gsolFile() {
        return _tempFileName+".gsol";
    } // end gsolFile
            ...
            if (CmplTools::stringStartsWith(line,"%opt")) {
                ...
                while (iss) {
                    ...
                    if (!subs.empty() && !CmplTools::stringStartsWith(subs,"%opt")) {
                        ...
                        if( !(tmpOsol.solver=="cbc" || tmpOsol.solver=="glpk" || tmpOsol.solver=="glpsol"|| tmpOsol.solver=="gurobi" || tmpOsol.solver=="scip" || tmpOsol.solver=="cplex") )
                ...
                if (tmpOsol.solver=="glpk" || tmpOsol.solver=="glpsol")
    ...
    void CmplArgs::writeSolverOpts() throw (ArgError) {
        ...
        } else if (_solverName=="glpk" || _solverName=="glpsol" ) {

            for (int i=0; i<osolList.size(); i++) {
                if (osolList[i].solver=="glpk" || osolList[i].solver=="glpsol")
                    _solverOpts+=" "+osolList[i].name + " " +osolList[i].value;
            }
    ...
    int CmplArgs::parseArgs()  throw (ArgError) {
       ...
       // tmpNr = qrand() % ((999999 + 1) - 1) + 1;
        QTemporaryFile f;
        f.open();
        _tempFileName= f.fileName().toStdString();
        f.close();
        setDefaults();
        ...
        if ( argList.size()==2 && !CmplTools::stringStartsWith(cArg, "-")  ) {
            ...
            mps.setFileName(_tempFileName+".mps");
            // mps.setFileName(QDir::tempPath().toStdString()+"/"+QString::number(tmpNr).toStdString()+".mps");
           // mps.setFileName(QDir::tempPath().toStdString()+"/"+CmplTools::modelName(_cmplFile)+".mps");
        ...
        for (  int i = 1; i < argList.size(); i++ ) {
            ...
            if (cArg=="-solver") {
                ...
                if (_solverName != "cbc" && _solverName != "glpk" && _solverName != "glpsol" && _solverName != "glpsol" && _solverName != "scip"  && _solverName != "gurobi" && _solverName != "cplex")
            ...
            if (cArg=="-send" || cArg=="-knock" || cArg=="-retrieve" || cArg=="-cancel" ) {
                continue;
            }
        ...
        if (_asciiName.empty() ) {
            if (_alias.empty())
                _asciiName= CmplTools::problemName(_cmplFile)+".sol";
