...
bool cfResult::setProblem(QString &fname, cmpl::CmplCore *m, CmplInstance *c) {
    ...
    osilName=QString::fromStdString(glvar->args->osil.fileName());
    osrlName=QString::fromStdString(glvar->args->osrlFile());
...
void cfResult::writeCmplSolutionHeader() {
    ...
    cout << "   <general>" << endl;
    cout << "       <instanceName>" <<  instanceName() << "</instanceName>" << endl;
...
bool cfResult::writeCmplSol() {
    ...
    if (!report.open()) {
        setCfOsMessage("Internal Error - Can not write the cmplSolution file!\n" );
        return false;
    }
