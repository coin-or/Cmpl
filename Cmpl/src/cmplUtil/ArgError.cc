...
namespace cmplUtil {
    ...
    void ArgError::usage(int mode) {
        ...
        uOut << " -cmplUrl <url>          : Url of a CmplServer - Without other arguments, the problem are solved on the CmplServer (synchronous mode)" << endl;
        uOut << " -send                   : Sends a problem to a CmplServer which have to be specified with -cmplUrl (asynchronous mode)" << endl;
        uOut << " -knock                  : Obtains the status of a problem at the CmplServer (asynchronous mode)" << endl;
        uOut << " -retrieve               : Retrieves the results of the problem from the CmplServer (asynchronous mode)" << endl;
        uOut << " -cancel                 : Cancels the problem at the CmplServer (asynchronous mode)" << endl;
