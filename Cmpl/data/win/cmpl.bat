...
FOR %%x IN (%*) DO (
	...
	if "!mArg!"=="-send" set asyncMode=1
	if "!mArg!"=="-knock" set asyncMode=2
	if "!mArg!"=="-retrieve" set asyncMode=3
	if "!mArg!"=="-cancel" set asyncMode=4
...
if %cmplFileFound%==1 ( 
	...
	for /f "eol=# tokens=2-3" %%a in ('findstr "%%args send" "!filename!"') do (
		if "%%a"=="-send" (
			set asyncMode=1
		)
	)

	for /f "eol=# tokens=2-3" %%a in ('findstr "%%args knock" "!filename!"') do (
		if "%%a"=="-knock" (
			set asyncMode=2
		)
	)

	for /f "eol=# tokens=2-3" %%a in ('findstr "%%args retrieve" "!filename!"') do (
		if "%%a"=="-retrieve" (
			set asyncMode=3
		)
	)

	for /f "eol=# tokens=2-3" %%a in ('findstr "%%args cancel" "!filename!"') do (
		if "%%a"=="-cancel" (
			set asyncMode=4
		)
	)
...
if %isRemote%==1 (
	...
	!PythonBin! "%CmplProgPath%pyCmpl\scripts\cmplOnServer.py" !cmplFile! !cmplUrl! !maxServerTries! !maxServerQueuingTime! !asyncMode! !cArgList!
