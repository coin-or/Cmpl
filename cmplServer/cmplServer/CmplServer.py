...
class CmplServer(object):
	...
	def startServer(self): 
		try:
			...
			logMsg="CmplServer has been started"
			if self.__serverMode==CMPL_GRID:
				logMsg+=" in CmplGrid"
			logMsg+=" | port: "  + str(self.__cmplPort) + " | maxProblems: " + str(self.__maxProblems ) + " | maxInactivityTime: " + str(self.__maxInactivityTime) + " | serviceIntervall: " + str(self.__serviceIntervall)
			logMsg+=" | solvers: " + str(self.__solvers)
			CmplServerTools.cmplLogging( self.__logFile, logMsg )	
	...
	def __cmplSchedulerExecute(self, server, method="", *args):
		...
		while True:
			try:
				...
				if tries==self.__maxCmplSchedulerTries:
